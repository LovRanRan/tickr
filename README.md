...
> **Status**: ✅ MVP complete with 42 passing tests. See [`../Progress.md`](../Progress.md) for the
> stage-by-stage development log.
...
2. For each `PortfolioEntry`, schedule `BinanceProvider.fetch_quote(symbol)`
3. `asyncio.gather(*coros)` runs all of them concurrently inside a single
   `async with httpx.AsyncClient()` block
4. Each provider parses its API's JSON and constructs a `Quote` —
...
│       ├── binance.py      # crypto provider — MVP
│       └── yahoo.py        # stock provider — Stretch (planned)
└── tests/
    ├── conftest.py
...
| **pytest + pytest-asyncio + respx** | `pytest-asyncio` lets test functions be `async def`. `respx` mocks `httpx` at the transport level so tests don't hit the real Binance API — entire 42-test suite runs in ~0.2s, fully offline. |
...
wall time for sequential vs concurrent execution against Binance's `/ticker/24hr`
endpoint.

```
Refreshing 5 USDT pairs (BTC/ETH/SOL/DOGE/ADA) against Binance:
  sequential (await one at a time):   3.94 s
  concurrent (asyncio.gather):        0.88 s
  speedup:                            4.5×
```

**4.5× / theoretical max 5× ≈ 90% utilization.** The remaining 10% gap is
DNS + TLS handshake overhead from creating a fresh `httpx.AsyncClient` per
`fetch_quote` call — a future Stretch optimization (shared client via async
context manager) would close that gap.

The sequential baseline is an explicit `for symbol in symbols: await
provider.fetch_quote(symbol)` loop; the concurrent version is the production code
path used by `tickr refresh`. Reproduce with `python _bench.py` from the project
root. Numbers will vary with network conditions; the **ratio** is the interesting
bit, and it confirms the workload is IO-bound (so `asyncio.gather` is the right
tool — `multiprocessing` wouldn't help).

> Note: an earlier draft used CoinGecko's free public API but its ~30 req/min
> rate limit broke the A/B benchmark. Switching to Binance (~3000 req/min for
> single-symbol queries) gave a clean comparison and aligns with the long-term
> goal of building this `BaseProvider` abstraction toward a real trading agent.
...
|---|---|
| OOP classes + class variables | `Quote`, `PortfolioEntry`, `Portfolio`, `BaseProvider` |
| Inheritance | `BaseProvider` → `BinanceProvider` (and future `YahooProvider`) |
...
- ✅ **Stage 1** — `models.py` + `exceptions.py` (Pydantic data shapes + exception tree)
- ✅ **Stage 2** — `BaseProvider` ABC + `BinanceProvider` (async core, **4.5× speedup**)
- ✅ **Stage 3** — `Portfolio` + `storage.py` (OOP dunders + JSON persistence + cost-weighted merge)
- ✅ **Stage 4** — `cli.py` wires everything through `add` / `list` / `refresh` end-to-end
- ✅ **Stage 5** — 42-test pytest suite (offline via `respx`), README, Git polish
- ⬜ **Stretch** — `YahooProvider` (stocks), `rich` table output, GitHub Actions CI, shared `httpx.AsyncClient`, exponential-backoff retry
